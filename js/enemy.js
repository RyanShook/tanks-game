...
class EnemyBomber {
    constructor(scene, position) {
        this.scene = scene;
        this.mesh = this.createBomberMesh();
        this.mesh.position.copy(position);
        this.mesh.position.y = 15 + Math.random() * 5;
        this.health = 100;
        this.isDestroyed = false;
        this.lastBombTime = Date.now();
        this.bombInterval = 3000; // Time in ms between dropping bombs
        this.speed = 0.04;
        this.direction = new THREE.Vector3(1, 0, 0); // Start moving along the x-axis
        scene.add(this.mesh);
    }
    ...
    createBomberMesh() {
        ...
        const points = [];
        // Fuselage
        points.push(new THREE.Vector3(0, 0, -4), new THREE.Vector3(0, 0, 4));
        // Wings
        points.push(new THREE.Vector3(-3, 0, 0), new THREE.Vector3(3, 0, 0));
        // Tail
        points.push(new THREE.Vector3(0, 0, 4), new THREE.Vector3(0, 1, 3));
        // Wing tips
        points.push(new THREE.Vector3(-3, 0, 0), new THREE.Vector3(-2.5, 0, 0.5));
        points.push(new THREE.Vector3(3, 0, 0), new THREE.Vector3(2.5, 0, 0.5));
        const geometry = new THREE.BufferGeometry().setFromPoints(points);
        const bomber = new THREE.LineSegments(geometry, new THREE.LineBasicMaterial({ color: VECTOR_GREEN }));
        group.add(bomber);
        return group;
    }
    ...
    update() {
        ...
        // Move bomber horizontally at constant speed
        this.mesh.position.add(this.direction.clone().multiplyScalar(this.speed));
        
        // Turn around at world boundaries
        if (Math.abs(this.mesh.position.x) > GAME_PARAMS.WORLD_BOUNDS - 5) {
            this.direction.x *= -1;
        }
        if (Math.abs(this.mesh.position.z) > GAME_PARAMS.WORLD_BOUNDS - 5) {
            this.direction.z *= -1;
        }
    ...
    destroy() {
        this.isDestroyed = true;
        createEnhancedExplosion(this.mesh.position, VECTOR_GREEN, 3);
        this.scene.remove(this.mesh);
    }

    dropBomb() {
        if (this.isDestroyed) return;
        const bomb = projectilePool.acquire();
        bomb.visible = true;
        bomb.position.copy(this.mesh.position);
        bomb.position.y -= 1; // Drop from below bomber
        
        // Bombs fall straight down with slight randomness
        const fallVelocity = new THREE.Vector3(
            (Math.random() - 0.5) * 0.1, // Slight horizontal drift
            -0.3, // Fast downward fall
            (Math.random() - 0.5) * 0.1
        );
        
        bomb.userData.velocity = fallVelocity;
        bomb.userData.distanceTraveled = 0;
        bomb.userData.creationTime = Date.now();
        bomb.userData.isEnemyProjectile = true;
        bomb.userData.isBomb = true; // Special flag for bombs
        state.projectiles.push(bomb);
        
        // Visual effect when dropping
